...
class ArrivalRemoteDataSourceImpl(
    ...
    override suspend fun getRideNotification(stationName: String, exitName: String): RideNotificationResponse {
        return try {
            val response = arrivalService.getRideNotification(stationName, exitName)
            if (response.resultCode == "SUCCESS") {
                response.result
            } else {
                Log.e("실패", "서버 요청 실패: ${response.resultCode}")
